...
def load_and_process_files(file_list, context_window_size):
    temp = []
    ...
    if temp:
        df_final = pd.concat(temp, ignore_index=True)
        print(f"\nCombined dataframe shape: {df_final.shape}")
    else:
        print("No files were loaded!")

    return df_final

train_df = load_and_process_files(train_xlsx, context_window_size)
print(train_df.head())

train_df = train_df[['previous_context', 'student_utterance', 'turn', 'subsequent_context', 'filename', 'student_tag']]
train_df.to_csv('asking_for_more_info/full_train_df.csv', index=False)
...
# repeat for test set
test_df = load_and_process_files(test_xlsx, context_window_size)
test_df = test_df[['previous_context', 'student_utterance', 'turn', 'subsequent_context', 'filename', 'student_tag']]
test_df.to_csv('asking_for_more_info/full_test_df.csv', index=False)